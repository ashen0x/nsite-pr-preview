...
```bash
npx github:ashen0x/nsite-pr-preview          # build and report what would be published
npx github:ashen0x/nsite-pr-preview --live   # publish it and comment the link on the PR
```

Run it from inside any repo, on any branch you have already pushed as a `pr/` branch. It installs dependencies, runs your build and stamps the output as a preview. Nothing is committed. Each person signs their own preview with their own key, so a maintainer never hands out credentials.
...
That step needs you to approve in your signer app. A `nostr.nsec` in your git config or a `NSITE_SEC` environment variable also work, and those need no approval so they run unattended. ngit's own `nostr.bunker-uri` does not work, because it carries no secret parameter for a second client to connect with.
...
| | |
|---|---|
| `--live` | publish, rather than reporting what would be published |
...
```bash
npx github:ashen0x/nsite-pr-preview --build "cd ui && pnpm install && pnpm build" --dir ui/out
```
...
A live run prints the preview url on stdout and everything else on stderr, so `URL=$(npx github:ashen0x/nsite-pr-preview --live)` captures just the link.
